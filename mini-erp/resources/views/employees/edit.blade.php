...
<head>
    ...
    <title>تعديل موظف | وحدة التحكم</title>
...
<body class="min-h-screen flex items-center justify-center p-6 selection:bg-white selection:text-black">
    ...
    <div class="w-full max-w-2xl border-4 border-white p-8 md:p-12 relative bg-[#0a0a0a]">
        ...
        <header class="mb-10 border-b-2 border-gray-800 pb-4">
            <span class="mono text-xs text-gray-500 tracking-widest uppercase block mb-1">ACTION // UPDATE_RECORD</span>
            <h2 class="text-4xl font-extrabold uppercase">تعديل <span class="text-gray-400">موظف.</span></h2>
        </header>
        ...
        <form action="{{ route('employees.update', $employee->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block mono text-xs uppercase tracking-widest text-gray-400 mb-2">الرقم السري (اتركه فارغاً إذا لا تريد تغييره)</label>
                    <input type="password" name="password" class="brutal-input mono">
